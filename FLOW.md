...
7. statusItem 생성 (28pt, config.statusBarIcon에 따라 StatusBarIcon template 또는 bolt.fill 심볼).
   Lightning + statusBarAnimation != off면 StatusIconAnimator가 CPU 연동 애니메이션 시작
...
상태바 아이콘과 번개 아이콘의 CPU 애니메이션(Off/Pulse/Wobble)을 관리한다.
...
| 상태바 애니메이션 (`StatusIconAnimator`) | main | 애니메이션이 켜진 동안만 CPU 3초 샘플링 + 1~20fps 프레임 타이머. 접근성 경고·Off·Default 아이콘이면 정지 |
...
### 이슈 2 — 앱 최소 창 크기보다 작게 요청하면 `partial` (동작상 정상, 안내 추가됨)

Outlook을 831x519로 요청한 2건이 `partial`이었다. 앱이 자체 최소 크기로 클램프한 결과이며 버그가 아니다.
사용자가 1300x769로 올려 해소됐다. App 타입은 이제 클램프가 감지되면
(`MinimumSizeNoticePolicy`) 앱 최소 크기와 현재 설정 크기를 alert로 안내한다.
같은 launchable·크기 조합은 앱 세션당 한 번만 표시하고, 설정 크기를 바꾸면 다시 안내한다.
